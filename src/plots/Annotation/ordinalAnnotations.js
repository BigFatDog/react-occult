...
export const getColumnScreenCoordinates = ({
  ...
  const positionValue =
    ...
    ['swarm', 'ordinalpoint', 'clusterbar'].find(p => p === type.type)
      ...
      ? max(
          pieces.map(p => (p.value >= 0 ? p.scaledValue + p.bottom : p.bottom))
        )
      : min(
          pieces.map(p => (p.value >= 0 ? p.bottom - p.scaledValue : p.bottom))
        );
...
export const htmlFrameHoverRule = ({
  ...
  if (d.isSummaryData) {
    ...
    const summaryLabel = (
      <p key="html-annotation-content-2">{summaryContent}</p>
    );
    contentFill = [
