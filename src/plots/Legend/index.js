...
const renderHorizontalGroup = ({ legendGroups, title, height }) => {
  let offset = 0;
  ...
  legendGroups.forEach((l, i) => {
    ...
    const renderedItems = renderLegendGroupHorizontal(l);
...
const renderLegendGroupHorizontal = legendGroup => {
...
const renderGroup = ({ legendGroups, width }) => {
  let offset = 30;
...
const renderLegendGroup = legendGroup => {
  const { type = 'fill', styleFn, items } = legendGroup;
  const renderedItems = [];
  let itemOffset = 0;
  items.forEach((item, i) => {
