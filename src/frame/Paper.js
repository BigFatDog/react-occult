...
import computeXYFrame from './computeXYFrame';
...
const isOrdinal = type => [Timeline, OrdinalPoint, Bar, ClusterBar, BarPercent].includes(type);
...
const Paper = props => {
    ...
    const xyChildren = React.Children.toArray(children)
        .filter(d => isXY(d.type));

    const ordinalChildren = React.Children.toArray(children)
        .filter(d => isOrdinal(d.type));

    let frameData = null;
    if (xyChildren.length > 0) {
        frameData = computeXYFrame({...props, axesDefs, plotChildren: xyChildren});
